...
import numpy as np
import argparse
from src.dataset.CadSeqProc.sequence.transformation.extrude_sequence import ExtrudeSequence
from src.dataset.CadSeqProc.sequence.transformation.revolve_sequence import RevolveSequence
from src.dataset.CadSeqProc.sequence.sketch.sketchsequence import SketchSequence
from src.dataset.CadSeqProc.utility.logger import CLGLogger
from src.dataset.CadSeqProc.utility.macro import *
from src.dataset.CadSeqProc.utility.decorator import *
from src.dataset.CadSeqProc.utility.utils import (
    add_axis,
    add_padding,
    brep2mesh,
    get_files_scan,
...
from src.dataset.CadSeqProc.OCCUtils.Topology import Topo
...
class CADSequence(object):
    """
    A Cad Model Sequence which consists of sketch and operation sequences.

    Requires:

    sketch_seq: List of SketchSequence objects
    extrude_seq: List of ExtrudeSequence objects
    revolve_seq: List of RevolveSequence objects (可选)
    bbox: optional
    """
    ...
    def __init__(
        ...
        revolve_seq: List[RevolveSequence] = None,
        bbox=None,
        operation_order=None
    ):
        self.sketch_seq = sketch_seq
        self.extrude_seq = extrude_seq
        self.revolve_seq = revolve_seq if revolve_seq is not None else []
        self.bbox = bbox
        self.operation_order = operation_order if operation_order is not None else []
    ...
    def create_variables(self):
        ...
        self.revolve_vec = []  # 新增:Revolve sequence in vector format
    ...
    @staticmethod
    def from_vec(cad_vec, bit=N_BIT, post_processing=False, denumericalize=True, fix_collinearity=False):
        """根据向量序列重建CAD模型，正确处理混合操作顺序"""
        ...
        # 解析操作序列
        curr_pos = 0
        while curr_pos < len(cad_vec):
            # 查找下一个草图结束标记
            sketch_end = curr_pos
            while sketch_end < len(cad_vec) and cad_vec[sketch_end, 0] != END_TOKEN.index("END_SKETCH"):
                sketch_end += 1
                
            if sketch_end >= len(cad_vec):
                break  # 没有找到草图结束标记
                
            # 提取草图部分
            sketch_part = cad_vec[curr_pos:sketch_end+1]
            curr_pos = sketch_end + 1
            ...
            # 查找操作结束标记
            while op_end < len(cad_vec):
                if cad_vec[op_end, 0] == END_TOKEN.index("END_EXTRUSION"):
                    is_extrude = True
                    break
                elif cad_vec[op_end, 0] == END_TOKEN.index("END_REVOLVE"):
                    is_revolve = True
                    break
                op_end += 1
                
            if op_end >= len(cad_vec):
                break  # 没有找到操作结束标记
                
            # 提取操作部分
            op_part = cad_vec[curr_pos:op_end+1]
            curr_pos = op_end + 1
            
            # 创建草图
            sketch_obj = SketchSequence.from_vec(sketch_part, bit, post_processing, fix_collinearity)
            sketch_idx = len(sketch_seq)
            sketch_seq.append(sketch_obj)
            
            if is_extrude:
                # 处理拉伸操作
                extrude_obj = ExtrudeSequence.from_vec(op_part, bit, post_processing)
                extrude_idx = len(extrude_seq)
                extrude_seq.append(extrude_obj)
                
                # 设置坐标系统
                sketch_obj.coordsystem = extrude_obj.coordsystem
                
                # 记录操作顺序
                operation_order.append({
                    "type": "extrude",
                    "extrude_index": extrude_idx,
                    "sketch_index": sketch_idx
                })
                
            elif is_revolve:
                # 处理旋转操作
                revolve_obj = RevolveSequence.from_vec(op_part, bit, post_processing)
                revolve_idx = len(revolve_seq)
                revolve_seq.append(revolve_obj)
                
                # 设置坐标系统
                sketch_obj.coordsystem = revolve_obj.coordsystem
                
                # 记录操作顺序
                operation_order.append({
                    "type": "revolve",
                    "revolve_index": revolve_idx,
                    "sketch_index": sketch_idx
                })
        ...
        cad_seq = CADSequence(
            ...
            revolve_seq=revolve_seq,
            operation_order=operation_order
        )
        
        # 是否执行数值反转换
        if denumericalize:
            return cad_seq.denumericalize(bit=bit)
        else:
            return cad_seq
    ...
    def to_vec(self, padding=False, max_cad_seq_len=MAX_CAD_SEQUENCE_LENGTH):
        """
        将CAD序列转换为向量表示，同时创建标记向量和索引向量
        
        Args:
            padding: 是否添加填充使向量长度一致
            max_cad_seq_len: 最大CAD序列长度
            
        Returns:
            self: 对象自身，方便链式调用
        """
        self.cad_vec = [[END_TOKEN.index("START"), 0]]
        self.flag_vec = [0]  # 令牌类型
        self.index_vec = [0]  # 草图-操作索引
        
        # 如果有存储的操作顺序，按照该顺序处理
        if self.operation_order:
            for op in self.operation_order:
                sketch_idx = op["sketch_index"]
                skt_vec = self.sketch_seq[sketch_idx].to_vec()
                
                # 根据操作类型处理
                if op["type"] == "extrude":
                    ext_idx = op["extrude_index"]
                    ext_vec = self.extrude_seq[ext_idx].to_vec()
                    
                    # 添加草图向量
                    self.cad_vec.extend(skt_vec)
                    self.flag_vec.extend([EXTRUDE_FLAG_START] * len(skt_vec))
                    self.index_vec.extend([ext_idx + 1] * len(skt_vec))
                    
                    # 添加拉伸向量
                    self.cad_vec.extend(ext_vec)
                    self.flag_vec.extend([EXTRUDE_FLAG_RANGE[min(ext_idx, 9)]] * len(ext_vec))
                    self.index_vec.extend([ext_idx + 1] * len(ext_vec))
                    
                elif op["type"] == "revolve":
                    rev_idx = op["revolve_index"]
                    rev_vec = self.revolve_seq[rev_idx].to_vec()
                    
                    # 添加草图向量
                    self.cad_vec.extend(skt_vec)
                    self.flag_vec.extend([REVOLVE_FLAG_START] * len(skt_vec))
                    self.index_vec.extend([rev_idx + 1] * len(skt_vec))
                    
                    # 添加旋转向量
                    self.cad_vec.extend(rev_vec)
                    self.flag_vec.extend([REVOLVE_FLAG_RANGE[min(rev_idx, 16)]] * len(rev_vec))
                    self.index_vec.extend([rev_idx + 1] * len(rev_vec))
        else:
            # 兼容旧版本，没有操作顺序的情况
            # 处理拉伸操作
            for i, (skt, ext) in enumerate(zip(self.sketch_seq, self.extrude_seq)):
                skt_vec = skt.to_vec()
                ext_vec = ext.to_vec()
                ...
                self.cad_vec.extend(skt_vec)
                self.flag_vec.extend([EXTRUDE_FLAG_START] * len(skt_vec))
                self.index_vec.extend([i + 1] * len(skt_vec))
                
                # 添加拉伸向量
                self.cad_vec.extend(ext_vec)
                self.flag_vec.extend([EXTRUDE_FLAG_RANGE[min(i, 9)]] * len(ext_vec))
                self.index_vec.extend([i + 1] * len(ext_vec))
                
            # 处理旋转操作
            rev_count = min(len(self.revolve_seq), len(self.sketch_seq))
            for i in range(rev_count):
                skt = self.sketch_seq[i]
                rev = self.revolve_seq[i]
                ...
                # 添加旋转向量
                self.cad_vec.extend(rev_vec)
                self.flag_vec.extend([REVOLVE_FLAG_RANGE[min(i, 16)]] * len(rev_vec))
                self.index_vec.extend([i + 1] * len(rev_vec))
        ...
        if padding:
            ...
            if num_pad > 0:  # 只在需要填充时执行
                self.cad_vec = add_padding(self.cad_vec, num_pad)
                self.flag_vec += [PADDING_FLAG] * num_pad
    ...
    def json_to_NormalizedCAD(data: dict, bit: int):
        """
        创建已标准化但草图部分反标准化的CAD模型，兼容拉伸和旋转操作
        ...
        Returns:
            CADSequence: 标准化的CAD序列对象，草图部分已反标准化
        """
        # 创建CADSequence对象
        cad_seq = CADSequence.from_dict(data)
        ...
        # 根据操作顺序处理草图反标准化
        if hasattr(cad_seq, 'operation_order') and cad_seq.operation_order:
            # 使用操作顺序信息
            for op in cad_seq.operation_order:
                sketch_idx = op["sketch_index"]
                # 跳过索引超出范围的情况
                if sketch_idx >= len(cad_seq.sketch_seq):
                    continue
                    
                skt = cad_seq.sketch_seq[sketch_idx]
                
                if op["type"] == "extrude":
                    # 拉伸操作
                    ext_idx = op["extrude_index"]
                    if ext_idx < len(cad_seq.extrude_seq):
                        ext = cad_seq.extrude_seq[ext_idx]
                        skt.denormalize(
                            bbox_size=ext.metadata["sketch_size"],
                            translate=0,
                            bit=bit,
                        )
                elif op["type"] == "revolve":
                    # 旋转操作
                    rev_idx = op["revolve_index"]
                    if rev_idx < len(cad_seq.revolve_seq):
                        rev = cad_seq.revolve_seq[rev_idx]
                        # 使用旋转操作中的sketch_size
                        if "sketch_size" in rev.metadata:
                            skt.denormalize(
                                bbox_size=rev.metadata["sketch_size"],
                                translate=0,
                                bit=bit,
                            )
                        else:
                            # 如果没有sketch_size，使用默认值
                            skt.denormalize(
                                bbox_size=1.0,
                                translate=0,
                                bit=bit,
    ...
    def from_dict(all_stat):
        """按照JSON文件中的操作顺序构建CADSequence"""
        sketch_seq = []
        extrude_seq = []
        revolve_seq = []
        operation_order = []  # 新增：用于存储操作的顺序

        # 先创建一个字典来映射每个操作对应的索引
        operation_map = {}

        # 第一次遍历序列：找出所有操作并记录它们在序列中的顺序
        seq_idx = 0
        for item in all_stat["sequence"]:
            if item["type"] == "Sketch":
                continue  # 跳过单独的草图条目，它们会在对应的操作中处理

            operation_map[item["entity"]] = seq_idx
            seq_idx += 1
        ...
        for item in all_stat["sequence"]:
            ...
            if item["type"] == "ExtrudeFeature":
                ...
                # 设置坐标系统
                setattr(extrude_seq[-1], "coordsystem", sketch_ops.coordsystem)
                ...
                operation_order.append({
                    ...
                    "extrude_index": extrude_idx,
                    "sketch_index": sketch_idx,
                    "sequence_index": operation_map.get(item["entity"], -1)
                })

            # 旋转操作处理
            elif item["type"] == "RevolveFeature":
                revolve_ops = RevolveSequence.from_dict(all_stat, item["entity"])
                uid_pairs = revolve_ops.get_profile_uids()
                if len(uid_pairs) == 0:
                    continue

                # 添加到旋转序列
                revolve_idx = len(revolve_seq)
                revolve_seq.append(revolve_ops)
                ...
                # 设置坐标系统
                setattr(revolve_seq[-1], "coordsystem", sketch_ops.coordsystem)
                revolve_seq[-1].add_info("sketch_size", sketch_ops.bbox_size)
        ...
        # 按序列索引排序操作
        operation_order.sort(key=lambda x: x["sequence_index"])
        ...
        # 创建CADSequence对象，并保存操作顺序
        cad_seq = CADSequence(sketch_seq, extrude_seq, revolve_seq, bbox)
        cad_seq.operation_order = operation_order
    ...
    def numericalize(self, bit=N_BIT):
        ...
        for rev_seq in self.revolve_seq:
            rev_seq.numericalize(bit=bit)
        return self
    ...
    def normalize(self, size=1, bit=N_BIT):
        ...
        # clglogger.debug(f"Global Normalization: {scale}")
        ...
        if self.operation_order:
            for op in self.operation_order:
                sketch_idx = op["sketch_index"]
                if sketch_idx >= len(self.sketch_seq):
                    continue

                skt = self.sketch_seq[sketch_idx]
                translate = add_axis(skt.start_point)

                if op["type"] == "extrude":
                    ext_idx = op["extrude_index"]
                    if ext_idx < len(self.extrude_seq):
                        ext = self.extrude_seq[ext_idx]
                        ext.transform(
                            translate=ext.coordsystem.rotate_vec(translate, translation=False)
                            - self.bbox[1],
                            scale=scale,
                        )
                        skt.normalize(translate=None, bit=bit)
                        skt.coordsystem = ext.coordsystem
                else:  # revolve
                    rev_idx = op["revolve_index"]
                    if rev_idx < len(self.revolve_seq):
                        rev = self.revolve_seq[rev_idx]
                        rev.transform(
                            coortranslate=rev.coordsystem.rotate_vec(translate, translation=False)
                            - self.bbox[1],
                            translate=-translate,
                            scale=scale,
                        )
                        skt.normalize(translate=None, bit=bit)
                        skt.coordsystem = rev.coordsystem
        return self
    ...
    def denumericalize(self, bit=N_BIT, post_processing=True):
        ...
        # 添加: 反量化旋转操作
        for rev in self.revolve_seq:
            rev.denumericalize(bit=bit, post_processing=post_processing)
            ...
            for op in self.operation_order:
                ...
                if op["type"] == "extrude":
                    ...
                    if rev_idx < len(self.revolve_seq):
                        ...
                        skt.coordsystem = rev.coordsystem
                        skt.denormalize(
                            bbox_size=rev.metadata.get("sketch_size", 1.0),
                            translate=0,
                            bit=bit
                        )
        return self
    ...
    def create_bbox_from_ext_vec(vec, bit, post_processing, mul):
        ...
        bbox_skt3d_e2 = bbox_skt3d - extent_two
    ...
    def get_bounding_box_per_model(self, mul=0.05):
        ...
        # 如果有操作顺序，按照操作顺序处理
        if hasattr(self, 'operation_order') and self.operation_order:
            for op in self.operation_order:
                sketch_idx = op["sketch_index"]
                ...
                if op["type"] == "extrude":
                    ...
                    # 创建2D草图边界框
                    bbox_sketch2d = np.array([[0, 0], [1, 1]]) * ext.metadata["sketch_size"]

                    # 转换到3D
                    rt_bbox = ext.coordsystem.rotate_vec(bbox_sketch2d)

                    # 创建3D边界框
                    bbox_skt3d = np.array([np.min(rt_bbox, axis=0), np.max(rt_bbox, axis=0)])

                    # 执行拉伸操作
                    extent_one = ext.metadata["extent_one"] * mul
                    extent_two = ext.metadata["extent_two"] * mul

                    normal = ext.coordsystem.metadata["z_axis"]
                    bbox_skt3d_e1 = bbox_skt3d + extent_one * normal
                    ...
                    # 为旋转操作创建边界框
                    # 这部分需要根据旋转体的具体参数计算
                    # 如果rev中包含sketch_size参数
                    if "sketch_size" in rev.metadata:
                        bbox_sketch2d = np.array([[0, 0], [1, 1]]) * rev.metadata["sketch_size"]
                    else:
                        # 使用一个默认值
                        bbox_sketch2d = np.array([[0, 0], [1, 1]])

                    # 转换到3D
                    rt_bbox = rev.coordsystem.rotate_vec(bbox_sketch2d)

                    # 创建3D边界框
                    bbox_skt3d = np.array([np.min(rt_bbox, axis=0), np.max(rt_bbox, axis=0)])

                    # 根据旋转角度和轴计算旋转体边界框
                    # 这是一个简化版本，实际应根据具体旋转参数计算
                    angle = rev.metadata.get("angle", 360)
                    if angle >= 360:  # 完全旋转
                        # 计算旋转半径（从轴到草图最远点的距离）
                        axis = rev.metadata.get("axis", np.array([0, 0, 1]))
                        center = rev.metadata.get("center", np.array([0, 0, 0]))

                        # 计算草图点到轴的最大距离
                        max_dist = 0
                        for point in bbox_skt3d:
                            # 计算点到轴的距离
                            v = point - center
                            dist = np.linalg.norm(v - np.dot(v, axis) * axis)
                        ...
                        # 部分旋转，需要更精确计算
                        # 这里使用简化版本
                        max_dim = np.max(np.abs(bbox_skt3d))
                        bbox_model = np.array([
    ...
    def create_cad_model(self):
        ...
        if hasattr(self, 'operation_order') and self.operation_order:
            for op in self.operation_order:
                if op["type"] == "extrude":
                    operations.append({
                        "type": "extrude",
                        "index": op["extrude_index"],
                        "sketch_index": op["sketch_index"]
                    })
                else:  # revolve
                    operations.append({
                        "type": "revolve",
                        "index": op["revolve_index"],
                        "sketch_index": op["sketch_index"]
                    })
        ...
        for op in operations:
            ...
            if sketch_idx >= len(self.sketch_seq):
                # 防止索引超出范围
                #clglogger.warning(f"跳过索引{sketch_idx}超出范围的操作")
                continue
                
            sketch = self.sketch_seq[sketch_idx]
            op_type = op["type"]
            op_idx = op["index"]
            
            # 根据操作类型处理
            if op_type == "extrude":
                ext = self.extrude_seq[op_idx]
                extrude_params = {
                    "extrude_values": [
                ...
                try:
                    ext_solid = sketch.build_body(extrude_params)
                except Exception as e:
                    clglogger.error(f"拉伸操作失败: {e}")
                    continue
                ...
                if solid_volume == 0:
                    clglogger.error(f"生成的实体体积为0，跳过该拉伸操作")
                    continue
                ...
                if set_op == "NewBodyFeatureOperation" or set_op == "JoinFeatureOperation":
                    cur_solid = ext_solid if cur_solid is None else perform_op(cur_solid, ext_solid, "fuse")
                elif set_op == "CutFeatureOperation":
                    if cur_solid is None:
                        clglogger.error(f"不能对空实体执行差集操作")
                        continue
                    cur_solid = perform_op(cur_solid, ext_solid, "cut")
                elif set_op == "IntersectFeatureOperation":
                    if cur_solid is None:
                        clglogger.error(f"不能对空实体执行交集操作")
                        continue
                    cur_solid = perform_op(cur_solid, ext_solid, "common")
                else:
                    clglogger.error(f"未知的布尔操作类型 {set_op}")
                    continue
                
            else:  # op_type == "revolve"
                rev = self.revolve_seq[op_idx]
                
                # 创建草图面
                try:
                    sketch_face = sketch.build_face()
                    # sketch_face = topods.Face(sketch_face)
                    normal = sketch.get_normal()
                    # sketch.debug_face(sketch_face, "revolve")
                except Exception as e:
                    clglogger.error(f"创建草图面失败: {e}")
                    continue
                
                # 创建旋转体
                try:
                    rev_solid = rev.build_body(sketch_face, normal)
                except Exception as e:
                    clglogger.error(f"旋转操作失败: {e}")
                    continue
                    
                # 检查体积
                props = GProp_GProps()
                brepgprop.VolumeProperties(rev_solid, props)
                solid_volume = props.Mass()
                if solid_volume == 0:
                    clglogger.error(f"生成的旋转体体积为0，跳过该旋转操作")
                    continue
                ...
                # 执行布尔操作
                boolean_type = rev.get_boolean()
                set_op = REVOLVE_OPERATIONS[boolean_type]
                
                if set_op == "NewBodyFeatureOperation" or set_op == "JoinFeatureOperation":
                    cur_solid = rev_solid if cur_solid is None else perform_op(cur_solid, rev_solid, "fuse")
                elif set_op == "CutFeatureOperation":
                    if cur_solid is None:
                        clglogger.error(f"不能对空实体执行差集操作")
                        continue
                    cur_solid = perform_op(cur_solid, rev_solid, "cut")
                elif set_op == "IntersectFeatureOperation":
                    if cur_solid is None:
                        clglogger.error(f"不能对空实体执行交集操作")
                        continue
                    cur_solid = perform_op(cur_solid, rev_solid, "common")
                else:
                    clglogger.error(f"未知的布尔操作类型 {set_op}")
                    continue
            ...
            # 检查是否生成了有效的实体
            if cur_solid is None:
                clglogger.error(f"布尔操作后没有生成有效的实体，恢复到之前的实体状态")
                cur_solid = prev_solid
                continue
            ...
            if prev_solid is not None:
                props1 = GProp_GProps()
                props2 = GProp_GProps()
                brepgprop.VolumeProperties(prev_solid, props1)
                brepgprop.VolumeProperties(cur_solid, props2)
                # if abs(props1.Mass() - props2.Mass()) < PRECISION:
                #     #clglogger.warning(f"布尔操作后实体体积没有变化，可能操作无效")

            # 检查实体是否有效
            analyzer = BRepCheck_Analyzer(cur_solid)
            if not analyzer.IsValid():
                clglogger.error(f"生成的实体无效，恢复到之前的实体状态")
                cur_solid = prev_solid
                continue
