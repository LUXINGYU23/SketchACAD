...
from src.dataset.CadSeqProc.geometry.line import Line
from src.dataset.CadSeqProc.geometry.curve import Curve
...
class Arc(Curve):
    ...
    def from_dict(arc_entity: dict):
        ...
        #     # 同时交换点坐标
        #     metadata["start_point"] = original_end
        #     metadata["end_point"] = original_start
        # is_small_arc = False
        ...
        # else:
        #     # 对于大圆弧，我们需要在反方向上取中间点
        #     mid_angle = (start_angle + end_angle + 2 * np.pi) / 2 % (2 * np.pi)
    ...
    def to_vec(self):
        ...
        #debug print origin and results
        # clglogger.debug(
        #     f"Arc to_vec: {coord_token} Start Point: {self.metadata['start_point']} Mid Point: {self.metadata['mid_point']} End Point: {self.metadata['end_point']}"
        # )
        return coord_token
    ...
    def from_vec(vec, bit=N_BIT, post_processing=False):
        ...
        metadata = {}
        # clglogger.debug(f"Original Vec: {vec}")
        ...
        arc.bit = bit
        # clglogger.debug(
        #     f"Arc from vec: {vec}, Start Point: {metadata['start_point']}, Mid Point: {metadata['mid_point']}, End Point: {metadata['end_point']}"
        # )
        return arc
    ...
    def get_angles_counterclockwise(self, eps=1e-8):
        ...
        # clglogger.debug(
        #     f"Angle S: {angle_s}, Angle M: {angle_m}, Angle E: {angle_e}"
        # )
    ...
    def build_body(self, coordsystem=None):
        ...
        # #print xyz of start, mid and end point
        # clglogger.debug(
        #     f"Start Point: {start_point.X()}, {start_point.Y()}, {start_point.Z()}"
        # )
        # clglogger.debug(
        #     f"Mid Point: {mid_point.X()}, {mid_point.Y()}, {mid_point.Z()}"
        # )
        # clglogger.debug(
        #     f"End Point: {end_point.X()}, {end_point.Y()}, {end_point.Z()}"
        # )
    ...
    def numericalize(self, bit=N_BIT):
        ...
        self.bit = bit
        # clglogger.debug(
        #     f"Numericalized: {self.metadata['start_point']}, {self.metadata['mid_point']}, {self.metadata['end_point']}"
        # )
        size = 2**bit - 1
        ...
        # clglogger.debug(
        #     f"Quantized: {self.metadata['start_point']}, {self.metadata['mid_point']}, {self.metadata['end_point']}"
        # )
    ...
    def denumericalize(self, bit=N_BIT):
        ...
        # clglogger.debug(
        #     f"Denumericalized: {self.metadata['start_point']}, {self.metadata['mid_point']}, {self.metadata['end_point']}"
        # )
