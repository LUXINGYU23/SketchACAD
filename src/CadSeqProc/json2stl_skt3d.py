...
def main():
    ...
    # For testing without a split_json file
    if not args.split_json:
        if not args.input_dir:
            clglogger.error("When split_json is not provided, input_dir must be specified")
            sys.exit(1)
            
        # Find a JSON file for testing
        json_files = glob.glob(os.path.join(args.input_dir, "*.json"))
        if not json_files:
            clglogger.error(f"No JSON files found in {args.input_dir}")
            sys.exit(1)
            
        # Use the first file for testing
        test_file = json_files[0]
        test_uid = os.path.basename(test_file).split('.')[0]
        all_json_uids = [test_uid]
        clglogger.info(f"Testing with file: {os.path.basename(test_file)}")
        process_one(test_file, args)
        clglogger.info(f"Processed file: {os.path.basename(test_file)}")
        return
